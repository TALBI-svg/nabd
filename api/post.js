...
export const getFeaturedPosts = async () => {
  ...
  export const getLatestPosts = async (limit, pageNu) => {
    try {
      const {data} = await client(`/post/posts?limit=${limit}&pageNu=${pageNu}`);
      return data
    } catch (error) {
      const {response} = error;
      if(response?.data){
  ...
  export const getSimilarPosts = async (id) => {
    try {
      const {data} = await client(`/post/similar-posts/${id}`);
      return data
    } catch (error) {
      const {response} = error;
      if(response?.data){
